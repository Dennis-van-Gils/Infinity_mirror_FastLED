...
void Adafruit_DotStar::hw_spi_init(void) { // Initialize hardware SPI
  ...
  SPI.setClockDivider(SPI_CLOCK_DIV2); // 8 MHz (6 MHz on Pro Trinket 3V)
    ...
    SPI.endTransaction();
  #else
    // This code runs for Arduino M0 pro
    SPI.setClockDivider(SPI_CLOCK_DIV16); // Keep the SPI clock at enough low speed
...
void Adafruit_DotStar::show(void) {
  ...
  if(dataPin == USE_HW_SPI) {
...
#else
    // Code runs here for Arduino M0 Pro
    ...
    //for(i=0; i<80; i++) spi_out(0xFF);    // end-frame marker
